...
class DeleteCMD {
  private:
	...
	void deleteRowAt(CreateTableCMD &tbl, int rowIndex) {
		if (rowIndex < 0 || rowIndex >= tbl.rowCnt) {
			throw "Invalid row index";
		}
		delete[] tbl.rows[rowIndex];
		for (int i = rowIndex; i < tbl.rowCnt - 1; i++) {
			tbl.rows[i] = tbl.rows[i + 1];
		}
		tbl.rows[tbl.rowCnt - 1] = nullptr;
		tbl.rowCnt--;
	}

	int deleteRowsWhere(CreateTableCMD &tbl, int colIdx, const string &value) {
		if (colIdx < 0 || colIdx >= tbl.columnCnt) {
			throw "Invalid column index";
		}
		int deleted = 0;
		for (int i = 0; i < tbl.rowCnt;) {
			if (tbl.rows[i][colIdx] == value) {
				deleteRowAt(tbl, i);
				deleted++;
			} else {
				i++;
  ...
  public:
	DeleteCMD *parse(const string &input) {
		...
		if (pos >= tokens->getTokenCount() || !isKeyword((*tokens)[pos], "WHERE")) {
			delete cmd;
			delete tokens;
			throw "DELETE requires a WHERE clause";
		}
		pos++;
